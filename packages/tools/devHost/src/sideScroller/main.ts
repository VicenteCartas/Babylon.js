...
export async function Main(_searchParams: URLSearchParams): Promise<void> {
    ...
    overlay.innerHTML = "← → Move &nbsp; Space Jump &nbsp; Shift Dash<br>Babylon.js 2D Side-Scroller Demo" +
    ...
    const parallaxLayers = createParallaxLayers(scene, LEVEL_WIDTH * TILE);
    ...
    // ─── Enemies ─────────────────────────────────────────────────────
    const enemies = createEnemies(scene, physics, TILE);

    // ─── Collectibles ────────────────────────────────────────────────
    const collectibles = createCollectibles(scene, TILE, lighting);
    ...
    engine.runRenderLoop(() => {
        ...
        debugText.text = `FPS: ${engine.getFps().toFixed(0)} | Pos: ${player.position.x.toFixed(0)},${player.position.y.toFixed(0)} | Ground: ${onGround}`;
        updateHUD();
...
function createParallaxLayers(scene: Scene2D, levelWidth: number): IParallaxLayer[] {
    const layers: IParallaxLayer[] = [];
    const layerDefs = [
        { color: new Color4(0.05, 0.05, 0.15, 1), factor: 0.1, count: 20, sizeMin: 60, sizeMax: 200, yRange: [0, 0.5] },
        { color: new Color4(0.08, 0.08, 0.2, 1), factor: 0.3, count: 15, sizeMin: 40, sizeMax: 120, yRange: [0.1, 0.7] },
        { color: new Color4(0.1, 0.1, 0.25, 1), factor: 0.5, count: 12, sizeMin: 30, sizeMax: 80, yRange: [0.2, 0.8] },
    ];
...
function createEnemies(scene: Scene2D, physics: PlanckPhysicsEngine, tileSize: number): IEnemy[] {
...
function createCollectibles(scene: Scene2D, tileSize: number, lighting: LightingManager2D): ICollectible[] {
