...
- Phase 10 is implemented: the `Node`/`ControlNode`/`SpriteNode` class hierarchy is replaced by a single flat plain-data type `AnimationNode` with a `_kind` discriminator (`"node" | "control" | "sprite"`) and standalone functions in `nodes/node.ts` (`CreateNode`, `CreateControlNode`, `CreateSpriteNode`, `UpdateNode`, `ResetNode`, `DecomposeWorldMatrixAtFrame`, `AttachSprite`, `GetNodeOpacity`, `SetNodeVisible`). All node kinds share one monomorphic object shape (kind-specific fields always present with defaults) to keep the per-frame update path monomorphic; `UpdateNode` folds in the former control visibility gating and sprite-driving behavior in the same order the inheritance dispatch produced. Per-track update closures (`animationFunctions`) and the existing keyframe storage format are kept as-is (no storage-format change yet). All consumers (`parsedTypes`, `spriteRecord`, `nullLayer`, `rasterization`, `buildAnimation`, the shape/solid/text layer features, `animationController`, `babylonSpriteAdapter`) call the standalone functions; diagnostic node-id strings are unchanged. Mechanical, shape-only refactor: unit tests pass and focused fixture visual goldens are expected unchanged.
...
### Phase 10 - Pure-data node graph (implemented)
